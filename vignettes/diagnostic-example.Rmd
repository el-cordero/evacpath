...
description: "Inspect each preparation and routing step before interpreting a model result."
...
vignette: >
  ...
  %\VignetteIndexEntry{Evacuation workflow: function-by-function diagnostics}
...
old_options <- options(width = 72)
diagnostic_summary <- utils::read.csv(system.file(
  "extdata/pkgdown-example-diagnostic-summary.csv", package = "evacpath"
))
...
The detailed code below is off by default so this guide remains practical to
build and browse. These static results come from the compact packaged run and
show the kinds of spatial and QA/QC outputs to review before a local rerun.

```{r diagnostic-inputs-figure, echo = FALSE, results = "asis"}
figure_html("pkgdown-example-inputs.png", "Example inputs and candidate safety exits.")
```

```{r diagnostic-time-figure, echo = FALSE, results = "asis"}
figure_html("pkgdown-example-time.png", "Modeled evacuation time to safety.")
```

```{r representative-result, echo = FALSE}
knitr::kable(time_summary, col.names = c("Modeled output", "Example value"))
```

```{r representative-diagnostics, echo = FALSE}
knitr::kable(diagnostic_summary, col.names = c("QA/QC check", "Result"))
```
...
```{r read-inputs, eval = run_diagnostics && example_data_available}
...
```{r raw-diagnostics, eval = run_diagnostics && example_data_available}
cat("DEM CRS:\n", terra::crs(dem_raw), "\n\n")
cat("Roads CRS:\n", terra::crs(roads_raw), "\n\n")
...
```{r plot-raw-inputs, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.2}
...
```{r clean-roads, eval = run_diagnostics && example_data_available}
road_exclude <- NULL
...
```{r prepare-tsunami-zones, eval = run_diagnostics && example_data_available}
...
```{r land-water-check, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.2}
oldpar <- par(mfrow = c(1, 2), mar = c(3, 3, 3, 5), las = 1)
...
```{r plot-tsunami-zones, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.2}
oldpar <- par(mfrow = c(1, 2), mar = c(3, 3, 3, 5), las = 1)
...
```{r crop-roads-inner-extent, eval = run_diagnostics && example_data_available}
...
```{r plot-roads-inner-extent, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.4}
...
```{r road-aware-escape-zone, eval = run_diagnostics && example_data_available}
...
```{r plot-road-aware-zone, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.4}
...
```{r compare-escape-points, eval = run_diagnostics && example_data_available}
escape_wrong <- try(
  find_escape_points(
...
```{r plot-escape-comparison, eval = run_diagnostics && example_data_available, fig.width = 7.5, fig.height = 2.4}
...
```{r assign-core-objects, eval = run_diagnostics && example_data_available}
hazard_zone <- zones$hazard_zone
escape_zone <- zones$escape_zone
dem <- zones$dem
roads <- roads_clean
...
```{r prepare-core-inputs, eval = run_diagnostics && example_data_available}
...
```{r make-grid, eval = run_diagnostics && example_data_available}
...
```{r plot-grid, eval = run_diagnostics && example_data_available}
...
```{r make-road-mask, eval = run_diagnostics && example_data_available}
...
```{r plot-road-mask, eval = run_diagnostics && example_data_available}
...
```{r make-road-origins, eval = run_diagnostics && example_data_available}
...
```{r plot-road-origins, eval = run_diagnostics && example_data_available}
...
```{r make-conductance, eval = run_diagnostics && example_data_available}
...
```{r plot-conductance, eval = run_diagnostics && example_data_available}
...
```{r sample-destinations, eval = run_diagnostics && example_data_available}
...
```{r calc-distance, eval = run_diagnostics && example_data_available && run_lcp}
...
```{r plot-distance-points, eval = run_diagnostics && example_data_available && run_lcp}
...
```{r calc-time, eval = run_diagnostics && example_data_available && run_lcp}
...
```{r make-polygons, eval = run_diagnostics && example_data_available && run_lcp}
...
```{r plot-evac-polygons, eval = run_diagnostics && example_data_available && run_lcp, fig.width = 7.5, fig.height = 3.2}
...
```{r run-wrapper, eval = run_diagnostics && example_data_available && run_lcp, message=FALSE, echo=FALSE}
result <- run_evacpath(
  hazard_zone = zones$hazard_zone,
  escape_zone = zones$escape_zone,
  roads = roads_clean,
...
```{r final-map, eval = run_diagnostics && example_data_available && run_lcp, fig.width = 7.5, fig.height = 3.4}
...
```{r write-outputs, eval = run_diagnostics && example_data_available && run_lcp}
write_evac_outputs(
  result,
