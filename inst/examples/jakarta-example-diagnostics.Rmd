---
title: "Jakarta evacuation workflow: function-by-function diagnostics"
...
params:
  ...
  road_exclude_field: "man_made"
  road_exclude_values: !r c("pier")
  grid_multiplier: 50
  dem_multiplier: 50
  road_buffer_m: 2
  escape_buffer_m: 5
...
vignette: >
  %\VignetteIndexEntry{Jakarta evacuation workflow: function-by-function diagnostics}
...
This vignette is a diagnostic walkthrough for the Jakarta tsunami-evacuation workflow. It is intentionally more detailed than a normal user vignette because its purpose is to test every major `evacpath` function independently before trusting the high-level `run_evacpath()` wrapper.

The Jakarta workflow has three important tsunami-specific complications:
...
jakarta_data_available <- all(input_files$exists)
jakarta_data_available
```

If `jakarta_data_available` is `FALSE`, check `params$data_dir` or move the three files into `evacpath/data-raw/`.
...
```{r read-inputs, eval = run_diagnostics && jakarta_data_available}
...
```{r raw-diagnostics, eval = run_diagnostics && jakarta_data_available}
cat("DEM CRS:\n", terra::crs(dem_raw), "\n\n")
cat("Roads CRS:\n", terra::crs(roads_raw), "\n\n")
...
```{r plot-raw-inputs, eval = run_diagnostics && jakarta_data_available, fig.height = 7}
par(mfrow = c(1, 2), mar = c(3, 3, 3, 5), las = 1)
...
This is where region-specific road filtering belongs. In the Jakarta prototype, piers were excluded because they can create misleading road-water intersections.

```{r clean-roads, eval = run_diagnostics && jakarta_data_available}
road_exclude <- NULL
...
```{r prepare-tsunami-zones, eval = run_diagnostics && jakarta_data_available}
...
```{r land-water-check, eval = run_diagnostics && jakarta_data_available, fig.height = 7}
par(mfrow = c(1, 2), mar = c(3, 3, 3, 5), las = 1)
...
```{r plot-tsunami-zones, eval = run_diagnostics && jakarta_data_available, fig.height = 7}
par(mfrow = c(1, 2), mar = c(3, 3, 3, 5), las = 1)
...
```{r crop-roads-inner-extent, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-roads-inner-extent, eval = run_diagnostics && jakarta_data_available, fig.height = 7}
...
```{r road-aware-escape-zone, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-road-aware-zone, eval = run_diagnostics && jakarta_data_available, fig.height = 7}
...
```{r compare-escape-points, eval = run_diagnostics && jakarta_data_available}
escape_wrong <- try(
  find_escape_points(
...
```{r plot-escape-comparison, eval = run_diagnostics && jakarta_data_available, fig.height = 8}
...
```{r assign-core-objects, eval = run_diagnostics && jakarta_data_available}
hazard_zone <- zones$hazard_zone
escape_zone <- zones$escape_zone
dem <- zones$dem
roads <- roads_clean
...
```{r prepare-core-inputs, eval = run_diagnostics && jakarta_data_available}
...
```{r make-grid, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-grid, eval = run_diagnostics && jakarta_data_available}
...
```{r make-road-mask, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-road-mask, eval = run_diagnostics && jakarta_data_available}
...
```{r make-road-origins, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-road-origins, eval = run_diagnostics && jakarta_data_available}
...
```{r make-conductance, eval = run_diagnostics && jakarta_data_available}
...
```{r plot-conductance, eval = run_diagnostics && jakarta_data_available}
...
This section is optional because it can be slow. For debugging, start with small `max_origins` and `max_destinations` values.

```{r sample-destinations, eval = run_diagnostics && jakarta_data_available}
...
```{r calc-distance, eval = run_diagnostics && jakarta_data_available && run_lcp}
...
```{r plot-distance-points, eval = run_diagnostics && jakarta_data_available && run_lcp}
...
plot(distance_points, "distance", add = TRUE, pch = 21, cex = 0.55)
```
...
```{r calc-time, eval = run_diagnostics && jakarta_data_available && run_lcp}
...
```{r make-polygons, eval = run_diagnostics && jakarta_data_available && run_lcp}
...
```{r plot-evac-polygons, eval = run_diagnostics && jakarta_data_available && run_lcp, fig.height = 7}
...
```{r run-wrapper, eval = run_diagnostics && jakarta_data_available && run_lcp}
result <- run_evacpath(
  hazard_zone = zones$hazard_zone,
  escape_zone = zones$escape_zone,
  roads = roads_clean,
...
```{r final-map, eval = run_diagnostics && jakarta_data_available && run_lcp, fig.width = 9, fig.height = 7}
...
par(mar = c(4, 4, 3, 5), mgp = c(2.2, 0.7, 0), las = 1, bg = "white", xpd = FALSE)
...
```{r write-outputs, eval = run_diagnostics && jakarta_data_available && run_lcp}
write_evac_outputs(
  result,
...
# 20. Legacy wrapper checks

The old names are retained as wrappers, but new work should use the clearer package names.

```{r legacy-checks, eval = run_diagnostics && jakarta_data_available}
legacy_grid <- evacuation_grid(hazard_zone, resolution = grid_resolution)
legacy_escape <- escape_points(escape_boundary_zone, roads_for_escape)
legacy_region <- region_area(hazard_zone, hazard_zone, buffer_m = 100)

assert_true(is_spatvector(legacy_grid), "legacy evacuation_grid() failed.")
assert_true(is_spatvector(legacy_escape), "legacy escape_points() failed.")
assert_true(is_spatvector(legacy_region), "legacy region_area() failed.")

list(
  legacy_grid_n = nrow(legacy_grid),
  legacy_escape_n = nrow(legacy_escape),
  legacy_region_n = nrow(legacy_region)
)
```
